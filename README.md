...
Além disso, no tutorial é utilizado o RabbitMQ sem o RabbitMQ Cluster Operator. Seguindo a recomendação oficial do RabbitMQ, utilizei o RabbitMQ Cluster Operator para gerar um cluster do RabbitMQ. Também a API do RabbitMQ passou a utilizar métodos async em C#. Como não é possível utilizar métodos async num construtor, a conexão da aplicação com RabbitMQ foi feita executando um método async no Program.cs.
